...
def runCntkModel(model, map_file, node_name = [], mb_size = 1):
    ...
    while sample_counts < len(imgPaths):
        ...
        if sample_counts % 100 < mb_size:
            print("Evaluating DNN (output dimension = {}) for image {} of {}: {}".format(len(feats[-1]), sample_counts, len(imgPaths), imgPaths[sample_counts-1]))
    data = [[imgPath, feat] for imgPath, feat in zip(imgPaths, feats)]
    return data


def runCntkModelAllImages(model, imgDict, imgDir, map_file, node_name = [], mb_size = 1):
    ...
    # Prepare cntk input
    imgLabelMap  = getImgLabelList(imgDict, imgDir)
    if len(imgLabelMap) == 0: # not a single image provided
        return dnnOutput
    ...
    # Store all dnnOutput in a dictionary
